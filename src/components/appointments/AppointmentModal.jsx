  ...
  async function handleSubmit(e) {
    ...
    if (error) { setError(error.message); setLoading(false) }
      ...
      if (!isEdit) {
        ...
        if (customer?.phone) {
          const fecha = new Date(form.scheduled_at).toLocaleDateString("es-EC", { timeZone: 'America/Guayaquil', weekday: "long", year: "numeric", month: "long", day: "numeric" }); 
          const hora = new Date(form.scheduled_at).toLocaleTimeString("es-EC", { timeZone: 'America/Guayaquil', hour: "2-digit", minute: "2-digit" }); 
          ...
          fetch('/api/send-confirmation', {
            ...
            supabase.from("automations").insert([{ 
              ...
              channel: "whatsapp", 
              message_preview: "Confirmacion de cita para " + customer.full_name, 
              status: result?.messages ? "sent" : "failed", 
              scheduled_for: new Date().toISOString(), 
              sent_at: new Date().toISOString(), 
              error_message: result?.error?.message || null 
            }]).then(r => console.log("Auto saved:", r)) 
          })
        }
      }
      onSaved()
